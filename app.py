...
def mevsimsellik_analizi(df):
    try:
        ...
        tr_gun = {'Monday': 'Pazartesi', 'Tuesday': 'Salı', 'Wednesday': 'Çarşamba', 'Thursday': 'Perşembe',
                  'Friday': 'Cuma'}
        ...
        with tab2:
            ...
            if st.button("Kayıt Ol", use_container_width=True):
                if yk and ys:
                    try:
                        conn = baglanti_kur();
                        conn.execute("INSERT INTO users VALUES (?,?)", (yk, sifrele(ys)))
                        conn.execute("INSERT INTO takip_listesi VALUES (?,?)", (yk, "ASELS.IS"))
                        conn.commit();
                        conn.close();
                        st.success("Başarılı!")
                    except:
                        st.warning("Alınmış isim.")
...
if not st.session_state['login_status']:
    ...
    conn = baglanti_kur()
    try:
        l = pd.read_sql("SELECT sembol FROM takip_listesi WHERE username=?", conn, params=(user,))['sembol'].tolist()
    except:
        l = ["ASELS.IS"]
    conn.close()
    if not l: l = ["ASELS.IS"]
    ...
    if not pdf.empty:
        ...
        st.sidebar.markdown(
            f"<div style='text-align:center;padding:15px;background:#222;border-radius:10px;margin-bottom:10px'><b>TOPLAM</b><br><span style='font-size:1.5em'>{t_val:,.0f} TL</span><br><span style='color:{'#0ecb81' if t_pl >= 0 else '#f6465d'}'>{t_pl:,.0f} TL</span></div>",
            unsafe_allow_html=True)
    ...
    if not df.empty and len(df) > 1:
        ...
        # AÇIKLAMALI TOOLTIP'LER GERİ GELDİ
        m2.metric("AI Sinyal", ml_y, f"%{ml_g:.0f} Güven",
                  help="Random Forest (Makine Öğrenmesi) modeli geçmiş 2 yıllık veriyi tarar. %50'den yüksekse sinyal üretir.")
        ...
        if 'Bollinger_Upper' in df:
            fig.add_trace(
                go.Scatter(x=df['Date'], y=df['Bollinger_Upper'], line=dict(color='gray', width=1), name='Üst Bant'))
            fig.add_trace(
                go.Scatter(x=df['Date'], y=df['Bollinger_Lower'], fill='tonexty', fillcolor='rgba(255, 255, 255, 0.05)',
                           line=dict(color='gray', width=1), name='Alt Bant'))

        if ai_df is not None:
            fig.add_trace(go.Scatter(x=ai_df['Date'], y=ai_df['Tahmin'], line=dict(color='yellow', dash='dot', width=2),
                                     name='AI Trend'))
            fig.add_trace(
                go.Scatter(x=ai_df['Date'], y=ai_df['Alt'], fill='tonexty', fillcolor='rgba(255, 255, 0, 0.15)',
                           line=dict(width=0), name='Güven Aralığı'))
        ...
        with st.expander("💰 İşlem Yap"):
            ad = st.number_input("Adet", 100.0);
            mal = st.number_input("Maliyet", son)
            if st.button("Kaydet"):
                conn = baglanti_kur()
        ...
        # --- SEKMELER (AÇIKLAMALAR & TÜRKÇE GERİ GELDİ) ---
        ...
        with tab1:
            ...
            with c1:
                ...
                if not md.empty: st.plotly_chart(
                    px.bar(x=md.index, y=md.values, color=md.values, color_continuous_scale='RdYlGn',
                           labels={'x': 'Gün', 'y': 'Getiri %'}), use_container_width=True)
            with c2:
                st.subheader("AI Detay")
                st.info(
                    f"🤖 **Random Forest Nedir?**\nGeçmiş veriyi tarayan yapay zeka modelidir.\n\n**Analiz Sonucu:**\nModel %{ml_g:.0f} ihtimalle **{ml_y}** bekliyor.")
        ...
        with tab2:
            ...
            st.plotly_chart(
                go.Figure(data=[go.Histogram(x=ret, nbinsx=40, marker_color='#3b8ed0')]).update_layout(height=300,
                                                                                                       template="plotly_dark"),
                use_container_width=True)

        with tab3:
            st.subheader("Geçmiş Veriler (Türkçe)")
            # TÜRKÇE ÇEVİRİ KISMI EKLENDİ
            tablo = df.copy()
            tablo = tablo.rename(
                columns={'Date': 'Tarih', 'Open': 'Açılış', 'High': 'Yüksek', 'Low': 'Düşük', 'Close': 'Kapanış',
                         'RSI': 'RSI Gücü'})
            st.dataframe(tablo[['Tarih', 'Açılış', 'Yüksek', 'Düşük', 'Kapanış', 'RSI Gücü']].sort_values('Tarih',
                                                                                                          ascending=False),
                         use_container_width=True, hide_index=True)

        with tab4:
            hf = st.number_input("Hedef Fiyat", son * 0.95);
            mail = st.text_input("Mail")
            if st.button("Kur"):
                if son <= hf:
                    mail_gonder(mail, secilen, son); st.success("Mail atıldı!")
                else:
                    st.warning("Düşmedi.")
        ...
        with tab5:
            ...
            if karsit:
                ...
                if not df2.empty:
                    df_comp = pd.merge(df[['Date', 'Close']], df2[['Date', 'Close']], on='Date',
                    ...
                    if not df_comp.empty:
                        ...
                        st.metric("Korelasyon (Benzerlik)", f"{corr:.2f}",
                                  help="1'e yakınsa birlikte hareket ediyorlar.")

                        fig_c = go.Figure()
                        fig_c.add_trace(go.Scatter(x=df_comp['Date'], y=df_comp['Norm_1'], mode='lines', name=secilen,
                                                   line=dict(color='#00ff00', width=2)))
                        fig_c.add_trace(go.Scatter(x=df_comp['Date'], y=df_comp['Norm_2'], mode='lines', name=karsit,
                                                   line=dict(color='#0000ff', width=2)))
                        fig_c.update_layout(height=400, template="plotly_dark", title="Getiri Farkı (%)")
                        st.plotly_chart(fig_c, use_container_width=True)
                    else:
                        st.warning("Tarih çakışması yok.")
                else:
                    st.error("Veri yok.")
